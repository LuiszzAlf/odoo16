...
class wizard_reportes_financieros(models.TransientModel):
    _name='tjacdmx_wizard_reportes_financierosv3'
    _description='Reportes financieros v3'
    ...
    def action_reportes_financieros(self, values):
        ...
        if self.reporte_select.no_reporte == 1:
            ...
            datas['form'] = res
            datas['anio_fiscal'] = int(self.anio_fiscal)
            return self.env['report'].get_action([], 'reportes.estadodeactividades_v3', data=datas)
        if self.reporte_select.no_reporte == 3:
            ...
            if self.periodo and self.anio_fiscal:
                self.env.cr.execute("""
                        ...
                        select
                            ...
                            a.periodo,
                            a.anio,
                            b.saldo_inicial as mes_anterior_si,
                            b.haber as mes_anterior_haber,
                        ...
                        where
                            a.periodo = %s
                            and a.anio = '%s'
                            and a.account_id in ('37','61','108','126','170','185','277','312','275','280','163','291','320','333','338','356','472','558','684','186','226','253','275','277','283','291','312','318','2635','123');  
                ...
                for i in rows_sql:
                    query_r = {
                        ...
                        'mes_consulta': i['mes_consulta'],
                        'mes_anterior': i['mes_anterior'],
                        'mes_anterior_si': i['mes_anterior_si'],
                        'mes_anterior_haber': i['mes_anterior_haber'],
                        'importe': i['importe'],
                        'origen': i['origen'],
                    }
                    ids.append(query_r)
                ...
                datas['form_data'] = {
                        ...
                        'anio_fiscal': int(self.anio_fiscal),
                        'anio_anterior': int(self.anio_fiscal)-1,
                        'periodo': self.periodo,
                        'periodo_anterior': 12 }
            return self.env['report'].get_action([], 'reportes.estado_cambios_situacion_financiera_v3', data=datas)
            ...
            datas['anio_fiscal']= self.anio_fiscal
            datas['periodo_fiscal']= self.periodo
            return self.env['report'].get_action([], 'reportes.pasivos_contingentes_v3', data=datas)
            ...
            return self.env['report'].get_action([], 'reportes.estado_analitico_deuda_v3', data=datas)
            ...
            if self.periodo and self.anio_fiscal:
                ...
                sql_activo = """with saldos_anterior as (
                                ...
                                where
                                    ...
                                    and a.account_id in (35, 36, 37, 38, 45, 52, 58, 61, 62, 72, 98, 108, 116, 117, 118,122, 126, 127, 137, 143, 147, 151, 161, 163, 164, 166, 170)
                                group by 1,2,3;"""
            ...
            datas['query_estado'] = ids
            datas['form'] = res
            return self.env['report'].get_action([], 'reportes.estado_analitico_activo_v3', data=datas)
        # Reporte Financiero - Estado de Variaciones en la Hacienda Publica / Patrimonio
            ...
            datas['form'] = res
            #
            return self.env['report'].get_action([], 'reportes.estado_variaciones_hpp_v3', data=datas)
            ...
            cuantas_bmoa="""(with saldos_anterior_ampli as (
                                ...
                                where
                                    a.anio = '%s'
                                    and a.periodo between 1 and %s
                                    and a.account_id in (126,163) group by 1,4)"""
            ...
            otros_origenenesf="""with saldos_anterior as (
                                    select
                                        b.saldo_final,
                                        b.account_id
                                    from
                                    ...
                                    select
                                        'oof' saldo_anterior,
                                        abs((sum(case when a.account_id in (62, 72, 98) then ABS(a.saldo_final) else 0 end))
                                        -(sum(case when a.account_id in (62, 72, 98) then ABS(b.saldo_final) else 0 end))
                                        +(sum(case when a.account_id in (108) then ABS(a.saldo_final) else 0 end))
                                        -(sum(case when a.account_id in (108) then ABS(b.saldo_final) else 0 end))) importe_saldo_anterior
                                    from
                                        v_balanza a
                                    join saldos_anterior b on
                                        b.account_id = a.account_id
                                    where
                                        a.anio = '%s'
                                        and a.periodo = 12
                                        and a.account_id in (108, 62, 72, 98);"""
            self.env.cr.execute(otros_origenenesf % (self.anio_fiscal-2,self.anio_fiscal-1))
            row_otros_origenenesf = self.env.cr.dictfetchall()
            saldos=[]
            saldos_flujos=[]
            ...
            for i in row_otras_inversiones:
                ...
                saldos_flujos.append(query_r)
            for i in row_otros_origenenesf:
                query_r = {
                    'tipo': i['saldo_anterior'],
                    'saldo': 0,
                    'saldo_anterior': i['importe_saldo_anterior'],
                    'saldo_total': 0
                }
                saldos_flujos.append(query_r)
            datas['saldos']= saldos
            ...
            return self.env['report'].get_action([], 'reportes.estado_flujos_efectivo_v3', data=datas)
            ...
            select
                a.code,
                a.account_id,
                (b.saldo_final) as saldo_inicial,
                (a.saldo_inicial) as saldo_inicial2,
                (a.debe) as debe,
                (a.haber) as haber,
                (bb.debe) as debe_anterior,
                (bb.haber) as haber_anterior,
                (a.saldo_final) as saldo_final
            from
                v_balanza a
            join saldos_anterior b on b.account_id = a.account_id
            join saldos_anterior_rdrea bb on bb.account_id = a.account_id
            where
                a.anio = '%s'
                and a.periodo =%s
                and a.account_id in (37,38, 45, 52,61, 62, 72, 98, 105, 108,123,126,127,
                137, 143, 147, 151, 161,163, 164, 167, 170, 186, 226, 253, 275, 277,280, 283, 291, 312, 318,317 , 2635, 
                333, 338, 356, 472, 558, 684,344,339,331,2635)
            order by
                code asc;"""

            self.env.cr.execute(sql_activo_circulante % (self.anio_fiscal-1,self.anio_fiscal-1,self.anio_fiscal,self.periodo))
            ...
            for i in row_activo_efectivo:
                query_r = {
                    ...
                    'code': i['code'],
                    'saldo_inicial': i['saldo_inicial'],
                    'saldo_inicial2': i['saldo_inicial2'],
                    'debe': i['debe'],
                    'haber': i['haber']*-1,
                    'debe_anterior': i['debe_anterior'],
                    'haber_anterior': i['haber_anterior']*-1,
                    'saldo_final':i['saldo_final'],
                }
                ids.append(query_r)
            datas['query_estado'] = ids
            datas['anio_fiscal']= self.anio_fiscal
            datas['periodo_fiscal']= self.periodo
            return self.env['report'].get_action([], 'reportes.estado_situacion_financiera_v3', data=datas)
        elif self.reporte_select.no_reporte == 11:
            arch=self.env['tjacdmx.notas.estaods.financieros'].search([('trimestre', '=', self.trimestre),('ejercicio', '=', self.anio_fiscal)])
            if(arch):
                base_url = self.env['ir.config_parameter'].get_param('web.base.url')
                attachment_obj = self.env['ir.attachment']
                attachment_id = attachment_obj.create(
                    {'name': arch.nota, 'datas_fname': arch.nota+'.pdf', 'datas': arch.archivo})
