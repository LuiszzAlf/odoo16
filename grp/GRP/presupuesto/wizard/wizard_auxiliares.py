...
def search_items(self,fecha_inicio,fecha_fin,cuenta_id,ids):
    ...
    t_abono=0
    t_saldo=0
    #raise ValidationError("%s" % (item_hijo))
    ...
    sql_all="""select distinct
                ...
                where aml.account_id=%s and aml.date between '%s' and '%s' and am.state in ('posted','cierre') order by 3"""
    self.env.cr.execute(sql_all % (cuenta_id,fecha_inicio,fecha_fin)) 
    rows_sql = self.env.cr.dictfetchall()


    sql_si_det = """select saldo_inicial from v_balanza where periodo=%s and account_id=%s"""
    self.env.cr.execute(sql_si_det % (periodo.month,cuenta_id))
    ...
    if(rows_sql):
        ...
        #raise ValidationError("%s" % (t_saldo_inicial))
    
    for i in rows_sql:
        sql_si_det = """select saldo_inicial from v_balanza where periodo=%s and account_id=%s"""
        self.env.cr.execute(sql_si_det % (periodo.month,i['account_id']))
        rows_sql_si_det = self.env.cr.dictfetchall()
        #raise ValidationError("%s" % (rows_sql_si_det[0]['saldo_inicial']))
        if(rows_sql_si_det):
            t_saldo_inicial=rows_sql_si_det[0]['saldo_inicial']
    ...
    #raise ValidationError("%s" % (tot_saldo_inicial))
...
def search_items_else(self,fecha_inicio,fecha_fin,cuenta_inicio,ids):
    ...
    for i in rows_sql:
        ...
        self.env.cr.execute(sql_si_det % (periodo.month,i['account_id']))
        rows_sql_si_det = self.env.cr.dictfetchall()
        #raise ValidationError("%s" % (rows_sql_si_det))
        # if(rows_sql_si_det):
        #     t_saldo_inicial=rows_sql_si_det[0]['saldo_inicial']
        t_cargo=t_cargo+i['cargo']
        t_abono=t_abono+i['abono']
...
class auxiliares_wizard(models.TransientModel):
    ...
    def get_decode_excel(self,fecha_reporte_var,rango_var,nombre_cuenta_var,tot_saldo_inicial_var,tot_cargo_var,tot_abono_var,tot_saldo_var, cuentas_var):
        ...
        for item in cuentas_var:       
            ...
            sheet.write_merge(n,n, 3, 3, 'Fecha', style_mov)
            sheet.write_merge(n,n, 4, 4, 'Cargo', style_mov)
            sheet.write_merge(n,n, 5, 5, 'Abono', style_mov)
            sheet.write_merge(n,n, 6, 6, 'Saldo', style_mov)
            n += 1
            if(item['movimientos']):
                for i in item['movimientos']:
                    ...
                    sheet.write_merge(n, n, 3, 3, i['fecha_mov_det'], style_item_money)
                    sheet.write_merge(n, n, 4, 4, i['cargo'], style_item_money)
                    sheet.write_merge(n, n, 5, 5, i['abono'], style_item_money)
                    sheet.write_merge(n, n, 6, 6, i['saldo'], style_item_money)
                    n += 1
            else:
